...
function UIController.OpenInventory()
    ...
    -- Fetch player inventory from server
            local function RefreshInventory()
                ...
                -- Request inventory from server via RemoteFunction
                local RequestInventory = ReplicatedStorage:FindFirstChild("MonsterMash"):FindFirstChild("RemoteEvents"):FindFirstChild("RequestInventory")
                if RequestInventory then
                    local success, data = pcall(function()
                        return RequestInventory:InvokeServer()
                    end)
                    if success and data and data.inventory then
                        for _, monster in ipairs(data.inventory) do
                            local cardFrame = RoundedFrame("Monster_" .. monster.uniqueId, UDim2.new(0, 550, 0, 50), UDim2.new(0, 5, 0, 5), COLORS.panel, scrollFrame)
                            local rarityColor = COLORS["rarity" .. monster.rarity] or COLORS.text
                            TextLabel("Name", UDim2.new(0, 180, 0, 25), UDim2.new(0, 10, 0, 2), monster.name, rarityColor, 16, Enum.Font.GothamBold, cardFrame)
                            TextLabel("Rarity", UDim2.new(0, 100, 0, 20), UDim2.new(0, 200, 0, 2), monster.rarity, rarityColor, 12, Enum.Font.Gotham, cardFrame)
                            TextLabel("EPS", UDim2.new(0, 80, 0, 20), UDim2.new(0, 300, 0, 2), "EPS: " .. tostring(monster.eps), COLORS.essence, 12, Enum.Font.Gotham, cardFrame)
                            TextLabel("Lvl", UDim2.new(0, 60, 0, 20), UDim2.new(0, 380, 0, 2), "Lv." .. tostring(monster.level), COLORS.textDim, 12, Enum.Font.Gotham, cardFrame)
                            local equipBtn = TextButton("Equip", UDim2.new(0, 80, 0, 28), UDim2.new(0, 450, 0, 10), monster.shiny and "Equipped" or "Equip", COLORS.success, cardFrame, function() end)
                            if monster.shiny then
                                equipBtn.Text = "★ Equipped"
                            end
                        end
                    else
                        TextLabel("Placeholder", UDim2.new(1, -10, 0, 40), UDim2.new(0, 5, 0, 10), "No monsters yet! Hatch some eggs.", COLORS.textDim, 16, Enum.Font.Gotham, scrollFrame)
                    end
                else
                    TextLabel("NoInventory", UDim2.new(1, -10, 0, 40), UDim2.new(0, 5, 0, 10), "Loading inventory...", COLORS.textDim, 16, Enum.Font.Gotham, scrollFrame)
                end
            end
...
function UIController.OpenBattleArena()
    ...
    -- Attack button (during battle)
            local attackBtn = TextButton("AttackBtn", UDim2.new(0, 100, 0, 40), UDim2.new(0.5, -50, 1, -60), "⚔️ Attack", COLORS.danger, menu, function()
                -- Send attack to server via PlayerAttack remote event
                local PlayerAttack = ReplicatedStorage:FindFirstChild("MonsterMash"):FindFirstChild("RemoteEvents"):FindFirstChild("PlayerAttack")
